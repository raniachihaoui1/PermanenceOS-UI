...
# saves each option as a JSON + PNG to the output folder.
#
# Usage: python tag_and_audit.py
# =============================================================================
...
MIN_GAP_GRID         = 1.49      # min gap for grid edge culling
MAX_GAP              = 5.0       # max span for layout option grouping
TOO_CLOSE            = 1.5       # two grid edges closer than this = potential pair
MID_PROXIMITY        = 1.0       # midpoint proximity for clustering
MIN_CHAIN_LEN        = 4.0       # wall chain length to be fixed (not culled)
MAX_BEAM_FOR_COL     = 2.2       # max beam length to consider culling a column
MIN_DIST_PROTECTED   = 1.5       # min distance from anchor to keep a column
protected_room_names = ["bedroom", "living"]  # room types whose corners are always kept
...
# BUILD OUTLINE + GRID
# =============================================================================
outline      = LineString(layout["outline"])
...
print(f"\nUnique X : {len(unique_x)}  Unique Y : {len(unique_y)}")
...
for pt in candidate_columns:
    ...
    is_room_corner = pt in room_corners
    ...
    if on_boundary:
        ...
        for i in matches:
            ...
            if is_corner(p_prev, p_curr, p_next):
                ...
                classified = True
                break
        if not classified and is_room_corner:
...
print(f"\nPerimeter corners   : {len(perimeter_corner_cols)}")
print(f"Collinear perimeter : {len(collinear_perimeter_cols)}")
print(f"Wall corners        : {len(wall_corner_cols)}")
print(f"Wall floating       : {len(wall_floating_cols)}")
...
print(f"\nPrimal graph — nodes: {G_primal.number_of_nodes()}  edges: {G_primal.number_of_edges()}")
...
print(f"Perimeter edges : {len(perimeter_edges)}")
print(f"Room wall edges : {len(room_wall_edges)}")
print(f"Grid edges      : {len(grid_edges_list)}")

# =============================================================================
# BASE REFERENCES
# =============================================================================
perimeter_nodes      = set(perimeter_corner_cols) | set(collinear_perimeter_cols)
perimeter_only_edges = [(u, v) for u, v in G_primal.edges()
...
wall_edge_dict      = {}
grid_edge_dict      = {}
for ek, t in edge_types.items():
    if t == 'perimeter':  perimeter_edge_dict[ek] = ek
    elif t == 'wall':     wall_edge_dict[ek]      = ek
    else:                 grid_edge_dict[ek]      = ek

print(f"\nEdge dicts — perimeter: {len(perimeter_edge_dict)}  wall: {len(wall_edge_dict)}  grid: {len(grid_edge_dict)}")

# =============================================================================
# WALL CULLING HELPERS
# =============================================================================
...
# WALL CULLING
# =============================================================================
perimeter_edges_list = list(perimeter_only_edges)
...
for i, edge in enumerate(candidate_wall):
    ...
    ray_results[i] = (
        ray_cast_distance(edge, d, ref_edges, -1),
        ray_cast_distance(edge, d, ref_edges, +1),
    )
    ray_results[i] = (*ray_results[i], ray_results[i][0] > MIN_GAP and ray_results[i][1] > MIN_GAP)

initial_culled = {i for i, r in ray_results.items() if not r[2]}
...
all_peri_wall = perimeter_edges_list + base_wall
...
final_wall_edges      = {tuple(sorted(e)): e for e in wall_options[0]}
final_grid_edges      = dict(grid_edge_dict)

print(f"\nWall layout options : {len(wall_options)}")
...
for wall_opt in wall_options:
    ...
    for node in corners:
        touching   = [(round(u[0],3), round(u[1],3)) if (round(u[0],3), round(u[1],3)) == node
                      else (round(v[0],3), round(v[1],3))
                      for u, v in base_wall_edges
                      if node_touches_edge(node, (u, v))]
        # rebuild properly
        touching = []
    ...
    if corner_opt_groups:
        ...
        expanded = [build_fixed(wall_opt, corner_forced, {i: c[i] for i in range(len(corner_opt_groups))})
                    for c in combos]
    else:
...
print(f"Wall options after corner fix : {len(wall_options)}")

# =============================================================================
# GRID CULLING HELPERS
# =============================================================================
...
def cull_grid_direction(edge_ids, edge_id_map, direction, fixed_edges):
    ...
    for cluster in clusters:
        ...
        col_sorted = sorted(col_candidates,
                            key=lambda eid: get_axis_val(*edge_id_map[eid], direction))
        cand_clusters = []
...
print(f"\nTotal layout combinations : {len(all_layout_options)}")
...
for wall_idx, grid_idx, wall_opt, grid_opt in all_layout_options:
    ...
    for u, v in final_kept:
        G_final.add_edge((round(u[0],3), round(u[1],3)), (round(v[0],3), round(v[1],3)))

    # cull collinear deg-2 nodes
    cull_nodes = set()
...
# =============================================================================
# BUILD STRUCTURE + SAVE PNG + JSON
# =============================================================================
for opt_idx, ld in enumerate(final_layouts):
    wall_idx   = ld['wall_idx']
    grid_idx   = ld['grid_idx']
    columns    = ld['columns']
    ...
    for u, v in sorted(beam_edges):
        ...
        u_peri    = u in perimeter_node_set
        v_peri    = v in perimeter_node_set
        beam_type = 'perimeter' if (u_peri and v_peri) else 'internal'
        beam_len  = round(math.dist(u, v), 3)
        structure.append({
            "id"        : beam_name,
    ...
    ax.scatter([], [], color="#E74C3C", s=100, label=f"Perimeter columns")
    ax.scatter([], [], color="#3A7FD5", s=100, label=f"Internal columns")
    ...
    png_name = f"{input_stem}_op{opt_idx+1}.png"
    ...
    # --- JSON ---
    output_json              = json.loads(json.dumps(layout))
    output_json['structure'] = structure
    json_name = f"{input_stem}_op{opt_idx+1}.json"
    json_path = os.path.join(output_dir, json_name)
    with open(json_path, 'w') as f:
        json.dump(output_json, f, indent=2)
    print(f"Saved JSON: {json_path}  ({n_cols} columns · {n_beams} beams)")

print(f"\nDone — {len(final_layouts)} options saved to {output_dir}")